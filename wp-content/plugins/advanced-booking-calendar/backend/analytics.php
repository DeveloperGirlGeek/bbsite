...
function advanced_booking_calendar_show_analytics() {
	...
	$er = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'advanced_booking_calendar_calendars ORDER BY name ASC', ARRAY_A);
	$foreachcount = 0;
	foreach($er as $row) {
	...
	if($foreachcount == 0){ // Check if there are calendars
		...
		$er = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'advanced_booking_calendar_bookings 
		...
		$output .= '
			<div class="uk-width-1-1">
				<div class="abc-analytics-wrapper ">
					<span class="abc-analytics-title"><h2>'.sprintf( __('Overall Occupation Rate for the next %d days', 'abc-bookings'), $numberOfDays ).'</h2></span>
		...
		$output .= '
			<div class="uk-width-1-1">
				<div class="abc-analytics-wrapper ">
					<span class="abc-analytics-title"><h2>'.sprintf( __('Occupation Rate of each calendar for the next %d days', 'abc-bookings'), $numberOfDays ).'</h2></span>
		...
		$requestRows = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'advanced_booking_calendar_requests 
		...
		$output .= '
			<div class="uk-width-1-1">
				<div class="abc-analytics-wrapper ">
					<span class="abc-analytics-title"><h2>'.sprintf( __('Number of requests for the next %d days', 'abc-bookings'), $numberOfDays ).'</h2></span>
		...
		$requestRows = $wpdb->get_results('SELECT persons, count(persons) as personCount FROM '.$wpdb->prefix.'advanced_booking_calendar_requests 
					WHERE date_from <= "'.$endDate.'" 
						AND date_to >= "'.$startDate.'" 
		...
		$output .= '
			<div class="uk-width-1-1 uk-width-medium-1-2">
				<div class="abc-analytics-wrapper ">
					<span class="abc-analytics-title"><h2>'.sprintf( __('Percentage per person for the next %d days', 'abc-bookings'), $numberOfDays ).'</h2></span>
		...
		$mainEr = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'advanced_booking_calendar_calendars ORDER BY name ASC', ARRAY_A);
		$outputBar = '';
		$revenueLabels = array();
		$revenueData = array();
		...
		$output .= '
			<div class="uk-width-1-1 uk-width-medium-1-2">
				<div class="abc-analytics-wrapper ">
					<span class="abc-analytics-title"><h2>'.sprintf( __('Average revenue per calendar for the next %d days', 'abc-bookings'), $numberOfDays ).'</h2></span>
