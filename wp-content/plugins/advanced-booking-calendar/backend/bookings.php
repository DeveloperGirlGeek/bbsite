...
function abc_booking_getBookingContent($state, $offset = 0, $itemsOnPage = 10, $sorting = 'standard') { // Returns table and accordion with all booking items, depending on the booking-state and the offset (used by ajax_getBookingContent())
	...
	$query = 'SELECT * FROM '.$wpdb->prefix.'advanced_booking_calendar_bookings WHERE state in ("'.$state.'") ORDER BY '.$where.' LIMIT '.$offset.', '.$itemsOnPage;
	...
	foreach($er as $row) {
		...
		$tables .= $buttons.'<form style="display: inline;" action="admin.php" method="get">
								<input type="hidden" name="page" value="advanced_booking_calendar" />
...
function abc_booking_editBookingContent() {
	...
	if(isset($_GET["id"]) && intval($_GET["id"]) > 0){
		...
		$nameQuery = $wpdb->get_row('SELECT r.name as roomName, c.name as calendarName 
					FROM '.$wpdb->prefix.'advanced_booking_calendar_rooms r
					INNER JOIN '.$wpdb->prefix.'advanced_booking_calendar_calendars c
		...
		$extrasResults = $wpdb->get_results('SELECT extra_id FROM '.$wpdb->prefix.'advanced_booking_calendar_booking_extras WHERE booking_id = '.$bookingId, ARRAY_A);
