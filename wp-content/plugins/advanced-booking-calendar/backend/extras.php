...
function abc_booking_addExtra() {
	...
	if ( isset($_POST["name"]) && isset($_POST["mandatory"]) && isset($_POST["calculation"]) 
		...
		$er = $wpdb->get_row('SELECT COUNT(id) as extras FROM '.$wpdb->prefix.'advanced_booking_calendar_extras', ARRAY_A);
		$extras = $er["extras"];
		if($extras < 2){
			...
			$wpdb->insert( $wpdb->prefix.'advanced_booking_calendar_extras', array(
				'name' 		  => $name,
...
function abc_booking_editExtra() {
	...
	if ( isset($_POST["id"]) && isset($_POST["name"]) && isset($_POST["mandatory"]) 
		...
		$price = sanitize_text_field($_POST["price"]);	
		$persons = intval($_POST["persons"]);
		$wpdb->update($wpdb->prefix.'advanced_booking_calendar_extras', array(
				'name' 		  => $name,
				'explanation' => $explanation,
				'calculation' => $calculation,
				'mandatory'	  => $mandatory,
...
function abc_booking_delExtra() {
	...
	if ( isset($_POST["id"]) ) {
		$wpdb->delete($wpdb->prefix.'advanced_booking_calendar_extras', array('id' => intval($_POST["id"])));
		$wpdb->delete($wpdb->prefix.'advanced_booking_calendar_booking_extras', array('extra_id' => intval($_POST["id"])));
...
function advanced_booking_calendar_show_extras() {
		...
		switch($_GET["setting"]){
			case 'extraLimit':
				$notices .= '<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible"> 
										<p><strong>'.__('Extras are limited to 2 in the free version! If you need more extras, please download our <a href="https://booking-calendar-plugin.com/pro-download/" target="_blank">Pro-Version</a>.', 'advanced-booking-calendar' ).'</strong></p><button type="button" class="notice-dismiss">
	...
	if(isset($_GET["action"]) && $_GET["action"] == "abc_booking_editExtra" && intval($_GET["id"]) > 0){
		
		// Does the ID exist?
		$row = $wpdb->get_row("SELECT COUNT(*) as co FROM ".$wpdb->prefix."advanced_booking_calendar_extras WHERE id = '".intval($_GET["id"])."'", ARRAY_A);
		if($row["co"] == 0) {
			// ID doesn't exist
			wp_die("Error! Unknown id<br />Please go <a href='admin.php?page=advanced-booking-calendar-show-extras'>back</a>");
		} else {
			//ID exists
			$row = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."advanced_booking_calendar_extras WHERE id = '".intval($_GET["id"])."'", ARRAY_A);
			$mandatoryYes = '';
			$mandatoryNo = '';
						...
						<table class="form-table">
						  <tr>
							...
							<td align="left"><input name="name" id="name" type="text" class="regular-text code" value="'.$row[0]["name"].'" required />
											 <p class="description">'.__('The name will be shown in the booking form.', 'advanced-booking-calendar').'</p></td>
						  </tr>
						  <tr>
							<td><label for="explanation">'.__('Explanation', 'advanced-booking-calendar').'</br><em>'.__('(optional)', 'advanced-booking-calendar').'</em></label></td>
						  ...
						  <tr>
							<td><label for="calculation">'.__('Type of calculation', 'advanced-booking-calendar').'</label></td>
							<td align="left">
								<select name="calculation" id="calculation">
									...
									<option value="personDay" '.$calculationPersonDay.'>'.__('per person per day', 'advanced-booking-calendar').'</option>
												 ...
												 <p class="description">'.__('Define how the price of the extra is getting charged.', 'advanced-booking-calendar').'</p></td>
						  </tr>
						  <tr>
							<td>'.__('Mandatory extra', 'advanced-booking-calendar').'</td>
							<td align="left">
								<input name="mandatory" id="mandatoryYes" type="radio" value="yes" '.$mandatoryYes.'> <label for="mandatoryYes">'.__('Yes', 'advanced-booking-calendar').'&nbsp;&nbsp;</label></input>
								<input name="mandatory" id="mandatoryNo" type="radio" value="no" '.$mandatoryNo.'> <label for="mandatoryNo">'.__('No', 'advanced-booking-calendar').'</label></input>
								<p class="description">'.__('You can make an extra mandatory, so every guest has to pay for it (eg. final cleaning).', 'advanced-booking-calendar').'</p></td>
						  </tr>
						  <tr>
							...
							<td align="left"><input value="'.$row[0]["price"].'" name="price" id="price" type="number" step="0.01" class="regular-text code" min="0.01" required />
												 <p class="description">'.__('Enter the price you want to charge for the extra.', 'advanced-booking-calendar').'</p></td>
						  </tr>
						  <tr>
							<td><label for="price">'.__('Persons', 'advanced-booking-calendar').'</label></td>
							<td align="left"><input value="'.$row[0]["persons"].'" name="persons" id="persons" type="number" step="1" class="regular-text code" min="1" required />
												 <p class="description">'.__('Enter the number of persons in a booking request to activate this extra. For example, if you enter &quot;4&quot;, the extra will be shown for <b>4 or more</b> persons.', 'advanced-booking-calendar').'</p></td>
						  </tr>
												 		
						</table>
						<br />
						<input class="button button-primary" type="submit" value="'.__('Save', 'advanced-booking-calendar').'" />
						<a href="admin.php?page=advanced-booking-calendar-show-extras"><input class="button button-secondary" type="button" value="'._x('Cancel', 'a change', 'advanced-booking-calendar').'" /></a>
						</form>
					  </div>';
			}	  
	}else{
		$extras = '';
		$er = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'advanced_booking_calendar_extras ORDER BY name', ARRAY_A);
		$foreachcount = 1;
		foreach($er as $row) {
			if ($foreachcount%2 == 1) {
		...
		$output .= ' <h3>'.__('Existing Extras', 'advanced-booking-calendar').'</h3>
					  <table class="wp-list-table widefat">
						<tr>
						  <td>'.__('Name', 'advanced-booking-calendar').'</td>
						  <td>'.__('Explanation', 'advanced-booking-calendar').'</td>
						  <td>'.__('Type of calculation', 'advanced-booking-calendar').'</td>
						  <td>'.__('Mandatory', 'advanced-booking-calendar').'</td>
						  <td>'.__('Price', 'advanced-booking-calendar').'</td>
						  <td>'.__('Persons', 'advanced-booking-calendar').'</td>
						  <td align="left"></td>
						</tr>
					  '.$extras.'
					  </table>
					  ...
					  <div class="wrap">
						...
						<table class="form-table">
						  <tr>
							<td><label for="name">'.__('Name', 'advanced-booking-calendar').'</label></td>
							<td align="left"><input name="name" placeholder="'.__('Wifi', 'advanced-booking-calendar').'" id="name" type="text" class="regular-text code" required />
											 <p class="description">'.__('The name will be shown in the booking form.', 'advanced-booking-calendar').'</p></td>
						  </tr>
						  <tr>
							<td><label for="explanation">'.__('Explanation', 'advanced-booking-calendar').'</br><em>'.__('(optional)', 'advanced-booking-calendar').'</em></label></td>
							<td align="left"><input placeholder="'.__('Get Wifi-access in your room.', 'advanced-booking-calendar').'" name="explanation" id="explanation" type="text" class="regular-text code"/>
												 <p class="description">'.__('Explain what the extra is for.', 'advanced-booking-calendar').'</p></td>
						  </tr>
						  <tr>
							<td><label for="calculation">'.__('Type of calculation', 'advanced-booking-calendar').'</label></td>
						  ...
						  <tr>
							<td>'.__('Mandatory extra', 'advanced-booking-calendar').'</td>
							<td align="left">
								<input name="mandatory" id="mandatoryYes" type="radio" value="yes"> <label for="mandatoryYes">'.__('Yes', 'advanced-booking-calendar').'&nbsp;&nbsp;</label></input>
								<input name="mandatory" id="mandatoryNo" type="radio" value="no" checked> <label for="mandatoryNo">'.__('No', 'advanced-booking-calendar').'</label></input>
								<p class="description">'.__('You can make an extra mandatory, so every guest has to pay for it (eg. final cleaning).', 'advanced-booking-calendar').'</p></td>
						  </tr>
						  <tr>
							<td><label for="price">'.__('Price', 'advanced-booking-calendar').'</label></td>
