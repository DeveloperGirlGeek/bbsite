...
function abc_booking_editCalendar() {
	...
	if ( isset($_POST["id"]) && isset($_POST["name"]) && isset($_POST["maxAvailabilities"]) 
		...
		$er = $wpdb->get_row('SELECT SUM(maxAvailabilities) as rooms FROM '.$wpdb->prefix.'advanced_booking_calendar_calendars', ARRAY_A);
		$rooms = $er["rooms"];
		$er = $wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'advanced_booking_calendar_calendars WHERE id = '.$_POST["id"], ARRAY_A);
			...
			for($i=$maxAvailabilities; $i< $er['maxAvailabilities']; $i++){
				$subEr = $wpdb->get_row('SELECT max(id) as roomId
								...
								AND id not in (
									...
									FROM '.$wpdb->prefix.'advanced_booking_calendar_bookings
									WHERE calendar_id = '.intval($_POST["id"]).')', ARRAY_A);
				if(isset($subEr['roomId'])){
					$wpdb->delete( $wpdb->prefix.'advanced_booking_calendar_rooms', array('id' => $subEr['roomId']));						
					$roomCounter--;
				}
			}
			$wpdb->update($wpdb->prefix.'advanced_booking_calendar_calendars', array('name'               => $name,
...
function abc_booking_delCalendar() {
	...
	if ( isset($_POST["id"]) ) {
		$er = $wpdb->get_row('SELECT count(id) as bookings FROM '.$wpdb->prefix.'advanced_booking_calendar_bookings WHERE calendar_id = '.intval($_POST["id"]), ARRAY_A);
		if($er["bookings"] > 0){ // There are still bookings for this calendar. Calendar can not be deleted.
			...
			$wpdb->delete($wpdb->prefix.'advanced_booking_calendar_seasons_assignment', array('calendar_id' => intval($_POST["id"])));
			abc_booking_setPersonCount();
...
function abc_booking_addSeason() {
	...
	if ( isset($_POST["title"]) && isset($_POST["price"]) && isset($_POST["minimumStay"]) && intval($_POST["minimumStay"]) >= 1 ) {
		$wpdb->insert( $wpdb->prefix.'advanced_booking_calendar_seasons', array(
...
function abc_booking_editSeason() {
	...
	if ( isset($_POST["id"]) && isset($_POST["title"]) && isset($_POST["price"]) && intval($_POST["minimumStay"]) >= 1) {
		$wpdb->update($wpdb->prefix.'advanced_booking_calendar_seasons', array(
			'title' => sanitize_text_field($_POST["title"]), 
			'price' => floatval($_POST["price"]),
			'minimumStay' => intval($_POST["minimumStay"])), 
			array('id' => intval($_POST["id"])));
...
function abc_booking_delSeason() {
	...
	if ( isset($_POST["id"]) ) {
		$wpdb->delete($wpdb->prefix.'advanced_booking_calendar_seasons', array('id' => intval($_POST["id"])));
		$wpdb->delete($wpdb->prefix.'advanced_booking_calendar_seasons_assignment', array('season_id' => intval($_POST["id"])));
	}
	...
	if (isset($_POST["calendar"]) && isset($_POST["season"]) && isset($_POST["dateStart"]) && isset($_POST["dateEnd"])) {
		...
		$query = "SELECT COUNT(*) as co FROM ".$wpdb->prefix."advanced_booking_calendar_seasons_assignment
				 				   WHERE calendar_id = '".sanitize_text_field($_POST["calendar"])."' 
				 				   		...
				 				   		AND season_id IN 
				 				   				(SELECT id FROM ".$wpdb->prefix."advanced_booking_calendar_seasons)";
	...
	if ( isset($_POST["id"]) && isset($_POST["calendar"]) && isset($_POST["season"]) && isset($_POST["dateStart"]) && isset($_POST["dateEnd"])) {
		...
		$row = $wpdb->get_results("SELECT COUNT(*) as co FROM ".$wpdb->prefix."advanced_booking_calendar_seasons_assignment
				 				   WHERE calendar_id = '".sanitize_text_field($_POST["calendar"])."' AND
										...
										AND (start < '".$normToValue."' AND end > '".$normFromValue."')
				 				   		AND season_id IN 
				 				   				(SELECT id FROM ".$wpdb->prefix."advanced_booking_calendar_seasons)", ARRAY_A);
		if($row[0]["co"] > 0) {
			wp_die("Overlap", "Seasons assignment", array('back_link' => true));
		}
...
function advanced_booking_calendar_show_seasons_calendars() {
		...
		switch($_GET["setting"]){
			case 'roomLimit':
				$notices .= '<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible"> 
										<p><strong>'.__('Number of rooms is limited to 15 rooms! If you need more rooms, please download our <a href="https://booking-calendar-plugin.com/pro-download/" target="_blank">Pro-Version</a>.', 'advanced-booking-calendar' ).'</strong></p><button type="button" class="notice-dismiss">
	...
	switch($getAction){
		case "abc_booking_editCalendar":
			// Does the ID exist?
			$row = $wpdb->get_row("SELECT COUNT(*) as co FROM ".$wpdb->prefix."advanced_booking_calendar_calendars WHERE id = '".intval($_GET["id"])."'", ARRAY_A);
			if($row["co"] == 0) {
				// ID doesn't exist
				wp_die("Error! Unknown id<br />Please go <a href='admin.php?page=advanced-booking-calendar-show-seasons-calendars'>back</a>");
			} else {
				//ID exists
				$row = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."advanced_booking_calendar_calendars WHERE id = '".intval($_GET["id"])."'", ARRAY_A);
		...
		case "abc_booking_editSeason":
			...
			if($row[0]["co"] == 0) {
				...
				//ID exists
				$row = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."advanced_booking_calendar_seasons WHERE id = '".intval($_GET["id"])."'", ARRAY_A);
