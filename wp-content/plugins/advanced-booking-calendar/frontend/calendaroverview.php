...
function abc_booking_getCalOverview ($atts) {
	...
	$maxAvailabilityQuery = $wpdb->get_results('SELECT calendar_id, count(calendar_id) as availability FROM '.$wpdb->prefix.'advanced_booking_calendar_rooms GROUP BY calendar_id ORDER BY calendar_id', ARRAY_A); 
...
function abc_booking_showCalOverview ( $atts ) {
	...
	if(isset($atts['legend']) && intval($atts['legend']) == 1){
		global $wpdb;
		$row = $wpdb->get_row('SELECT max(maxAvailabilities) as roomMax FROM '.$wpdb->prefix.'advanced_booking_calendar_calendars', ARRAY_A);
