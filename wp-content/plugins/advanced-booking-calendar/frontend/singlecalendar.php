...
function abc_booking_getCssForSingleCalendar($currentAvailability, $previousAvailability){
	$cssClass=' ';
	$cssClass = 'abc-booked ';
	...
	for ($i=1; $i<($maxday+$startday); $i++) {
		...
		if($i < $startday){
			...
			if(isset($availDates[date('Y-m-d', $newCurrentTime)]) && date('Y-m-d', $newCurrentTime)>= date('Y-m-d') ) {
				...
				if($cAvailability == 0){ 
					$cAvailability = '0';
					$cssClass .= abc_booking_getCssForSingleCalendar($cAvailability, $prevAvailability);
					$priceOutput .= '&nbsp;';
					$title = 'Fully booked';
